...
class fd_snn_t( nn.Module):
    ...
    def __init__( self, ft_size=1024, sep_sizes=[512], hop=256,
        dropout=0., adapt_fe=False, smoother=5, ortho=False):
    ...
    def forward( self, x):
        ...
        x = F.conv_transpose2d( x.unsqueeze(3), self.it.unsqueeze(3), stride=(self.hp,1), padding=(self.sz,0))
...
def main():
    ...
    net = fd_snn_t( ft_size=args.filters, hop=args.hop, smoother=args.smoother, sep_sizes=args.denoisersize, dropout=args.dropout, adapt_fe=not args.fourier, ortho=args.orthogonal)
    ...
    try:
        ...
        while it <= args.iterations:
            ...
            net.train()
            z,h = net( inps)
            loss = myloss( z, target, mse=(args.mse))
